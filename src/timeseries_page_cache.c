...
bool tsdb_build_page_cache(timeseries_db_t *db) {
  ...
  qsort(db->page_cache, db->page_cache_count, sizeof(timeseries_cached_page_t),
        page_offset_compare);
...
void tsdb_pagecache_add_entry(timeseries_db_t *db, uint32_t offset,
  ...
  if (db->page_cache_count == db->page_cache_capacity) {
    size_t newcap =
        (db->page_cache_capacity == 0) ? 8 : db->page_cache_capacity * 2;
    timeseries_cached_page_t *newarr =
        realloc(db->page_cache, newcap * sizeof(*newarr));
    if (!newarr) {
...
bool tsdb_pagecache_remove_entry(timeseries_db_t *db, uint32_t offset) {
  ...
  for (size_t i = 0; i < db->page_cache_count; i++) {
    if (db->page_cache[i].offset == offset) {
      ...
      if (remaining > 0) {
        memmove(&db->page_cache[i], &db->page_cache[i + 1],
                remaining * sizeof(db->page_cache[i]));
      }
      db->page_cache_count--;

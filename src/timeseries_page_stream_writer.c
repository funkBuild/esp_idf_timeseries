 ...
 */
static bool copy_flash_block(const esp_partition_t *part, uint32_t src_offset,
...
static bool relocate_region(timeseries_db_t *db, uint32_t old_offset,
  ...
  if (!timeseries_blank_iterator_next(&iter, &found_ofs, &found_sz)) {
    ESP_LOGE(TAG, "Failed to find blank region for size=%u",
  ...
  if (!copy_flash_block(db->partition, old_offset, found_ofs, old_size)) {
    ESP_LOGE(TAG, "Failed copying region 0x%08" PRIx32 " => 0x%08" PRIx32,
             old_offset, found_ofs);
    return false;
  }

  *out_new_offset = found_ofs;
  ESP_LOGV(TAG, "Relocated region 0x%08" PRIx32 " => 0x%08" PRIx32, old_offset,
           found_ofs);
  return true;
}
 ...
 */
static bool ensure_capacity(timeseries_page_stream_writer_t *writer,
                            size_t needed_bytes) {
  ...
  ESP_LOGV(TAG, "Growing region from %u => %u (relocation)...",
           (unsigned int)old_size, (unsigned int)new_size);

  if (!relocate_region(writer->db, old_offset, old_size, new_size,
                       &new_offset)) {
    return false;
  }
 ...
 */
static bool gorilla_flush_to_flash(void *context, const uint8_t *data,
                                   size_t len) {
  ...
  ESP_LOGV(TAG, "Flushing %zu bytes to flash @0x%08" PRIx32, len,
           writer->write_ptr);

  // Write to flash
  if (esp_partition_write(writer->db->partition, writer->write_ptr, data,
                          len) != ESP_OK) {
...
bool timeseries_page_stream_writer_init(timeseries_db_t *db,
  ...
  if (!timeseries_blank_iterator_next(&blank_iter, &region_ofs,
                                      &initial_size)) {
  ...
  // 2) Erase that region
  if (esp_partition_erase_range(db->partition, region_ofs, initial_size) !=
      ESP_OK) {
  ...
  hdr.sequence_num = ++db->sequence_num; // or you could delay this
  hdr.field_data_level = to_level;
  // hdr.page_size filled in later on finalize

  if (esp_partition_write(db->partition, region_ofs, &hdr, sizeof(hdr)) !=
      ESP_OK) {
...
bool timeseries_page_stream_writer_begin_series(
  ...
  if (!gorilla_stream_init(&writer->ts_stream, GORILLA_STREAM_INT,
                           ...
                           gorilla_flush_to_flash, &writer->ts_flush_ctx)) {
...
bool timeseries_page_stream_writer_finalize_timestamp(
  ...
  ESP_LOGV(TAG,
           ...
           (unsigned int)writer->ts_bytes, writer->ts_end_offset);
...
bool timeseries_page_stream_writer_write_value(
  ...
  default:
    ...
    ESP_LOGW(TAG, "Unsupported field type %d for streaming compression",
             (int)writer->series_field_type);
    return false;
  }
...
bool timeseries_page_stream_writer_end_series(
  ...
  ESP_LOGV(TAG, "Series %02X%02X%02X%02X: end_series => ts=%u, val=%u bytes",
           writer->fd_hdr.series_id[0], writer->fd_hdr.series_id[1],
           writer->fd_hdr.series_id[2], writer->fd_hdr.series_id[3],
           (unsigned int)col_hdr.ts_len, (unsigned int)col_hdr.val_len);

  if (esp_partition_write(writer->db->partition, writer->col_hdr_offset,
                          &col_hdr, sizeof(col_hdr)) != ESP_OK) {
  ...
  if (writer->fd_hdr.record_count == 0) {
    ESP_LOGW(TAG, "Series %02X%02X%02X%02X... has no points",
             writer->fd_hdr.series_id[0], writer->fd_hdr.series_id[1],
             writer->fd_hdr.series_id[2], writer->fd_hdr.series_id[3]);
  ...
  if (total_col_bytes > 0xFFFF) {
    ESP_LOGW(TAG, "Series chunk = %u bytes, exceeds 16-bit limit!",
             (unsigned int)total_col_bytes);
    return false;
  }
  writer->fd_hdr.record_length = (uint16_t)total_col_bytes;

  ESP_LOGW(TAG, "Series %02X%02X%02X%02X...: total=%u bytes",
           writer->fd_hdr.series_id[0], writer->fd_hdr.series_id[1],
           writer->fd_hdr.series_id[2], writer->fd_hdr.series_id[3],
           (unsigned int)writer->fd_hdr.record_length);

  ESP_LOGW(TAG, "Header offset=0x%08" PRIx32 ", col_hdr=0x%08" PRIx32,
           writer->fd_hdr_offset, writer->col_hdr_offset);

  ESP_LOGW(TAG, "Total records count: %u",
           (unsigned int)writer->fd_hdr.record_count);

  ESP_LOGW(TAG, "Start time: %" PRIu64 ", End time: %" PRIu64,
           writer->fd_hdr.start_time, writer->fd_hdr.end_time);

  // 7) Patch the field_data_header with the updated metadata
  if (esp_partition_write(writer->db->partition, writer->fd_hdr_offset,
                          &writer->fd_hdr, sizeof(writer->fd_hdr)) != ESP_OK) {
...
bool timeseries_page_stream_writer_finalize(
  ...
  timeseries_page_header_t hdr;
  if (esp_partition_read(writer->db->partition, writer->base_offset, &hdr,
                         sizeof(hdr)) != ESP_OK) {
  ...
  // Rewrite header
  if (esp_partition_write(writer->db->partition, writer->base_offset, &hdr,
                          sizeof(hdr)) != ESP_OK) {
    ESP_LOGE(TAG, "Failed patching final page header");
    return false;
  }

  ESP_LOGV(TAG, "Page created @0x%08" PRIx32 " => size=%" PRIu32,
